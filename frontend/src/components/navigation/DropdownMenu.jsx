          ...
          <AnimatePresence>
            {(open || isOpen) && (
              ...
              >
                    ...
                    <Menu.Item key={item.name}>
                      {({ active }) => (
                        <Link
                          to={item.path}
                          className={`${
                            active ? 'bg-cyan-500/20' : 'bg-cyan-500/10'
                          } group flex w-full items-center gap-3 rounded-lg px-3 py-3 text-sm text-cyan-400 transition-all duration-150`}
                        >
                ...
                {/* Featured Section */}
                <div className="px-1 py-2 space-y-1"> {/* Add space-y-1 class here */}
                    ...
                    <Menu.Item key={item.name}>
                      {({ active }) => (
                        <Link
                          to={item.path}
                          className={`${
                            active ? 'bg-indigo-500/20' : 'bg-indigo-500/10'
                          } group flex w-full items-center gap-3 rounded-lg px-3 py-3 text-sm text-indigo-400 transition-all duration-150`}
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={item.icon} />
                          </svg>
